...
export const initDevtools = (
  ...
  setTimeout(() => {
    devtools.send(
      null,
      initDevtoolState(initialStore, { autoPause, shouldStartLocked })
    );
  }, 100);
        ...
        case "SWEEP": {
          devtoolState.currentStateIndex =
            ...
            devtoolState.skippedActionIds.filter(
              ...
              (id: number) =>
                devtoolState.stagedActionIds.indexOf(id) <
                devtoolState.currentStateIndex
            ).length;
          // omit skipped actions computedStates
          devtoolState.computedStates = devtoolState.computedStates.filter(
            (_: any, index: number) => {
          ...
          devtoolState.skippedActionIds.map(
            (actionId: number) => delete devtoolState.actionsById[actionId]
          );
          // omit skipped actions stagedActionIds
          devtoolState.stagedActionIds = devtoolState.stagedActionIds.filter(
            (actionId: number) =>
              !devtoolState.skippedActionIds.includes(actionId)
          );
...
const toggleAction = (message: any, dispatch: any) => {
  ...
  for (
    let i = skipped ? currentStateIndex : currentStateIndex + 1;
    i < stagedActionIds.length;
    i++
  ) {
    const currentActionId = stagedActionIds[i];
    if (i !== currentStateIndex && skippedActionIds.includes(currentActionId))
...
const initDevtoolState = (
  initialStore: any,
  {
    autoPause = devtoolState.isPaused,
    shouldStartLocked = devtoolState.isLocked,
  }: any = {}
) => {
  devtoolState = {
    actionsById: {
